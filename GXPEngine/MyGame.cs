...
public class MyGame : Game {
	...
	float turnSpeedShip = 0.4f;
	float moveSpeedShip = 1;
	float characterSpeed = 0.6f;
	...
	public MyGame() : base(800, 600, false, true, -1, -1, false)     // Create a window that's 800x600 and NOT fullscreen
	{
		...
		character = new AnimationSprite("platformerPack_character.png", 4, 2);
		...
		soundTrack = new Sound("The_Endless_Journey.ogg", true, true).Play();
	...
	void MoveSpaceShip() {
		...
		if (Input.GetKey(Key.A)) {
			spaceShip.rotation -= turnSpeedShip * Time.deltaTime;
		}
		...
		if (Input.GetKey(Key.W)) {
			spaceShip.Move(0, -moveSpeedShip * Time.deltaTime); // move forward (=up)
		}
		if (Input.GetKey(Key.S)) {
			spaceShip.Move(0, moveSpeedShip * Time.deltaTime); // move backward (=down)
		}

		float shipsize = Mathf.Max(spaceShip.width/2, spaceShip.height/2);

        if (spaceShip.x < -shipsize)
        {
            spaceShip.x = width + shipsize;
        }
        if (spaceShip.x > width + shipsize)
        {
			spaceShip.x = -shipsize;
        }

        if (spaceShip.y < -shipsize)
        {
            spaceShip.y = width + shipsize;
        }
        if (spaceShip.y > width + shipsize)
        {
            spaceShip.y = -shipsize;
        }
    }
	...
	void MoveCharacter() {
		...
		if (Input.GetKey(Key.LEFT)) {
			character.x -= characterSpeed * Time.deltaTime;
			moving = true;
			character.Mirror(true, false);
            if (character.x < -100)
            {
                character.x = width + 100;
            }
        }
